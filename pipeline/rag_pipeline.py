...
class RagPipeline:
    ...
    def build_rag_with_memory(self, session_id: str):
        if not self.rag_chain:
            self.build_rag_chain()
        
        conversational_rag_chain = RunnableWithMessageHistory(
            self.rag_chain,
            lambda: get_session_history(session_id),
    ...
    def user_input(self, session_id: str, user_input: str):
        if not self.conversational_rag_chain:
            self.build_rag_with_memory(session_id)
        
        return self.conversational_rag_chain.invoke({"input": user_input})
